...
mod lit {
	use obfstr::wide;
	pub static ImagePath: &[u16] = wide!("ImagePath\0");
	pub static Type: &[u16] = wide!("Type\0");
	pub static Start: &[u16] = wide!("Start\0");
	pub static ErrorControl: &[u16] = wide!("ErrorControl\0");
	pub static CAPCOM_DEVICE: &[u16] = wide!("\\\\.\\Htsysm72FB\0");
	pub static SeLoadDriverPrivilege: &[u16] = wide!("SeLoadDriverPrivilege\0");
	pub static NT_SERVICES_PATH: &[u16] = wide!("\\Registry\\Machine\\System\\CurrentControlSet\\Services\\Htsysm72FB\0");
	pub static SLASH_CAPCOM_SYS: &[u16] = wide!("\\SysVAD.sys\0");
	pub static RECOVER_FILE_NAME: &[u16] = wide!("RECOVER\0");
}
use self::lit::*;
